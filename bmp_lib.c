...
void initialize_fheader(bmp_data* data){
    data->fheader.file_type[0] = 66; // B
    data->fheader.file_type[1] = 77; // M
    ...
    data->fheader.reserved1 = 0;
    data->fheader.imagedata_offset[0] = 54; // ファイルヘッダ+インフォヘッダ=54
...
void initialize_iheader(bmp_data* data){
    data->iheader.header_size = 40; // インフォヘッダのバイト数
    data->iheader.width = 640; // 横
    data->iheader.height = 360; // 縦
    data->iheader.plane_num = 1;
    data->iheader.bit_num = 24; // bit数
    data->iheader.compression_type = 0;
    data->iheader.imagedata_size = 0;
    data->iheader.horizon_dpi = 0;
    data->iheader.vertical_dpi = 0;
    data->iheader.color_index_num = 16777216; // 色数
